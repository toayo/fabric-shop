...
import { getProductImageUrl, getProductImageUrls } from "@/lib/product-images";
...
export default function ProductDetailPage({ params }: { params: { slug: string } }) {
  ...
  const productImages = getProductImageUrls(product.slug, product.images ?? []);
  ...
  const handleAdd = () => {
    ...
    addItem({
      ...
      name: product.name,
      image: getProductImageUrl(product.slug, product.images?.[0]),
      unit,
      length,
      ...
      <h1 className="mt-2 text-3xl font-semibold text-cocoa">{product.name}</h1>
          ...
          <div className="mt-6 space-y-4 rounded-3xl bg-white p-6 shadow-sm">
              ...
              {(["yard", "meter"] as const).map((option) => (
                <button
                  ...
                  className={`rounded-full px-4 py-2 text-sm font-semibold transition ${
                    unit === option
                      ? "bg-brand text-white"
                      : "border border-cocoa/20 text-cocoa hover:border-brand/40 hover:text-brand"
                  }`}
                >
            ...
            <motion.button
              whileTap={{ scale: 0.98 }}
              onClick={handleAdd}
              className="w-full rounded-full bg-brand px-6 py-3 text-sm font-semibold text-white shadow-soft transition hover:-translate-y-0.5 hover:bg-brand-dark"
            >
              Add to Cart
            </motion.button>
