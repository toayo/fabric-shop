...
export default function ProductCard({ product }: { product: FabricProduct }) {
  return (
    <motion.div
      ...
      className="group rounded-3xl bg-white/90 p-4 shadow-sm transition hover:-translate-y-1 hover:shadow-soft"
    >
      <Link href={`/product/${product.slug}`} className="block">
        <div className="relative aspect-[4/3] overflow-hidden rounded-2xl">
          <Image
            ...
            priority={false}
          />
          <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-cocoa/30 via-transparent to-transparent opacity-0 transition group-hover:opacity-100" />
        </div>
