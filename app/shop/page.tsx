...
export default function ShopPage() {
  ...
  const filtered = useMemo(() => {
    let items = [...products];
    if (type !== "all") {
      items = items.filter((item) => slugify(item.type) === slugify(type));
    }
    if (color !== "all") {
      items = items.filter((item) => slugify(item.color) === slugify(color));
    }
    if (query.trim()) {
      ...
      <div className="rounded-3xl bg-white p-8 shadow-sm">
          ...
          <div>
            <p className="text-xs uppercase text-cocoa/50">Price</p>
              <select
                ...
                className="mt-2 w-full rounded-full border border-cocoa/20 bg-white px-4 py-3 text-sm focus:border-brand/40 focus:outline-none focus:ring-2 focus:ring-brand/20"
              >
        ...
        <div className="mt-6">
          <input
            ...
            className="w-full rounded-full border border-cocoa/20 bg-white px-5 py-3 text-sm focus:border-brand/40 focus:outline-none focus:ring-2 focus:ring-brand/20"
          />
        </div>
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="mt-10">
          <EmptyState
            ...
            description="Try adjusting your filters or explore the full catalog."
          />
        </div>
      ) : (
